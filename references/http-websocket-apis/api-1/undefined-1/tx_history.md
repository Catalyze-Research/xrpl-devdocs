...
tx\_history 메소드는 가장 최근에 이루어진 트랜잭션 중 일부를 검색합니다.

{% hint style="info" %}
Caution:

이 메소드는 더 이상 사용되지 않으며 별도의 공지 없이 제거될 수 있습니다.
{% endhint %}
...
| `Field` | 유형       | 설명                   |
| ------- | -------- | -------------------- |
| `index` | 부호 없는 정수 | `start`요청에 사용된 값입니다. |
| `txs`   | 배열       | 거래 객체의 배열.           |

각 트랜잭션 객체에 포함된 필드는 트랜잭션 유형에 따라 약간씩 다릅니다. 자세한 내용은 트랜잭션 형식을 참조하세요.

## 발생 가능한 오류

* 일반적인 오류 유형입니다.
* invalidParams - 하나 이상의 필드가 잘못 지정되었거나 하나 이상의 필수 필드가 누락되었습니다.
* noPermission - 지정한 시작 필드가 10000보다 크지만 관리자로 서버에 연결되어 있지 않습니다.
