...
유효성 검사기 명령은 서버에서 사용하는 게시되고 신뢰할 수 있는 유효성 검사기의 현재 목록에 대한 사람이 읽을 수 있는 정보를 반환합니다.[![New in: rippled 0.80.1](https://img.shields.io/badge/New%20in-rippled%200.80.1-blue.svg)](https://github.com/ripple/rippled/releases/tag/0.80.1)

유효성 검사기 메소드는 권한이 없는 사용자가 실행할 수 없는 관리자 메소드입니다!
...
| Field                    | 유형  | 설명                                                                                                                                                |
| ------------------------ | --- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| local\_static\_keys      | 배열  | 구성 파일에서 명시적으로 신뢰하는 유효성 검사기에 대한 공개 키의 배열입니다.                                                                                                       |
| publisher\_lists         | 배열  | 게시자 목록 객체의 배열입니다. (자세한 내용은 아래를 참조하세요.)                                                                                                            |
| signing\_keys            | 객체  | 현재 신뢰하는 모든 유효성 검사기에 대해 마스터 공개 키에서 현재 임시 공개 키로 매핑하는 객체입니다. 임시 서명 키를 사용하지 않는 유효성 검사기는 제외됩니다.                                                        |
| trusted\_validator\_keys | 배열  | 현재 신뢰하는 모든 유효성 검사기의 마스터 공개 키 배열입니다.                                                                                                               |
| validation\_quorum       | 숫자  | 원장 버전을 검증하는 데 필요한 신뢰할 수 있는 유효성 검사의 최소 개수입니다. 상황에 따라 서버에서 더 많은 유효성 검사를 요구할 수 있습니다.                                                                 |
| validator\_list\_expires | 문자열 | 현재 검증자 목록이 만료될 때 사람이 읽을 수 있는 시간입니다. 서버가 아직 게시된 유효성 검사기 목록을 로드하지 않은 경우 문자열을 알 수 없거나 서버가 정적 유효성 검사기 목록을 사용하는 경우 문자열을 절대 사용할 수 없는 두 가지 특별한 경우가 있습니다. |

publisher\_lists 배열의 각 멤버는 다음 필드가 있는 게시자 목록 객체입니다:

| Field             | 유형       | 설명                                               |
| ----------------- | -------- | ------------------------------------------------ |
| available         | Boolean  | 거짓이면 목록의 유효성 검사기 키가 이 게시자에서 더 이상 지원되지 않을 수 있습니다. |
| expiration        | 문자열      | 이 게시된 목록이 만료될 예정일 때 사람이 읽을 수 있는 시간입니다.           |
| list              | 배열       | 목록에 있는 게시된 유효성 검사기 키의 배열입니다.                     |
| pubkey\_publisher | 문자열      | 목록 게시자의 Ed25519 또는 ECDSA 공개 키(16진수)입니다.          |
| seq               | 부호 없는 정수 | 이 게시된 목록의 시퀀스 번호입니다.                             |
| version           | 부호 없는 정수 | 목록 형식의 버전입니다.                                    |
